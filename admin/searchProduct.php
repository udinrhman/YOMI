...
if (isset($_POST['keyword'])) {
    ...
    if ($inputType != 'Stock') {
        if (!empty($keyword) && ($inputType == 'Manga' || $inputType == 'Light Novel')) {
            $queryProduct = "SELECT * FROM mangaln WHERE (title LIKE '%" . $keyword . "%' OR alternative_title LIKE '%" . $keyword . "%') AND type = '" . $inputType . "'ORDER BY mangaln_id DESC";
        } else if (!empty($keyword) && $inputType == 'All') {
            $queryProduct = "SELECT * FROM mangaln WHERE (title LIKE '%" . $keyword . "%' OR alternative_title LIKE '%" . $keyword . "%') ORDER BY mangaln_id DESC";
        } else if (empty($keyword) && ($inputType == 'Manga' || $inputType == 'Light Novel')) {
            $queryProduct = "SELECT * FROM mangaln WHERE type = '" . $inputType . "' ORDER BY mangaln_id DESC";
        } else if (empty($keyword) && $inputType == 'All') {
...
?>
        <h4 style="margin-left:15px;margin-top:20px;margin-bottom:0;font-weight:600;text-transform:uppercase"><?php echo $inputType ?></h4>
        <p class="card-text" style="color:#c0c0c0;float:left;margin-left:15px;"><?php echo $count ?> results for <strong style="text-transform:uppercase">
    ...
    <?php }
            ...
            if (!empty($keyword) && $inputType == 'Stock') {
                $count = 0;
                $query = "SELECT mangaln_id FROM mangaln WHERE (title LIKE '%" . $keyword . "%' OR alternative_title LIKE '%" . $keyword . "%') ORDER BY mangaln_id DESC";
                $result = mysqli_query($link, $query);
                while ($row = mysqli_fetch_array($result, MYSQLI_BOTH)) {
                    $countProduct = "SELECT mangaln.*FROM mangaln LEFT JOIN stock ON stock.mangaln_id = mangaln.mangaln_id WHERE (stock.mangaln_id = '$row[mangaln_id]' AND stock.stock='0') AND (mangaln.title LIKE '%" . $keyword . "%' OR mangaln.alternative_title LIKE '%" . $keyword . "%')
                ...
                $queryNoStock = "SELECT mangaln.*
                                        ...
                                        WHERE stock.mangaln_id IS NULL AND (mangaln.title LIKE '%" . $keyword . "%' OR mangaln.alternative_title LIKE '%" . $keyword . "%')
    ...
    ?>
        ...
        <p class="card-text" style="color:#c0c0c0;float:left;margin-left:15px;"><?php echo $count ?> results for <strong><?php echo $keyword ?></strong> products</p><br><br>
        <?php
